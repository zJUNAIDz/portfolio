...
export const projectsContent = {
  ...
  projects: [
    ...
    },
    {
      ...
      subtitle: "改ざん検知が可能なマルチテナント監査ログサービス",
      period: "2026年6月",
      summary:
        "監査イベントを追記専用かつ暗号学的に検証可能なトレイルとして記録する、セルフホスト可能な HTTP サービス。問うのは「何が起きたか」だけでなく「このレコードが完全で未改ざんだと信頼できるか」——素朴なログテーブルが静かに取りこぼす問いです。",
      highlights: [
        "各イベントを HMAC-SHA256 のハッシュチェーン(hash = HMAC(event ‖ prev_hash)、テナントごとの秘密鍵を使用)で 1 つ前のイベントに連結。1 行でも改ざん・削除・並び替えがあれば後続のリンクがすべて壊れ、1 回の検証呼び出しでチェーンを再走査して信頼が失われた正確なイベントを特定。SOC 2 / HIPAA / PCI-DSS に真にフィット。",
        "設計当初からマルチテナント:各テナントが独自の API キー、HMAC 秘密鍵、完全に分離されたチェーンを持つ。テナントのプロビジョニングは管理者用ルート配下に置き、API キーの参照を 5 分間キャッシュして、リクエストごとの認証で DB 往復を発生させない。",
        "取り込みレイテンシを抑える分割された書き込みパス:ハンドラはペイロードをインメモリのバッファ付きキューに入れて即座に 202 を返し、バックグラウンドのワーカープールがチェーン位置を計算して永続化。テナントごとの PostgreSQL アドバイザリロックにより、単一テナントのチェーンは厳密な順序を保ちつつ、異なるテナント同士は完全に並行して書き込み——グローバルロックなし。",
        "多層防御:events テーブルはすべての UPDATE/DELETE を拒否する PostgreSQL RULE によって DB レベルで追記専用となり、不具合のあるマイグレーションでも履歴を改変できない。ワーカーは指数バックオフでリトライし、シャットダウン時には処理中のリクエストとキューをドレインしてから終了。",
        "actor・action・resource・type・時間範囲によるリッチなフィルタリングとページネーション、さらに CSV と JSONL/NDJSON での行単位ストリーミングエクスポート。1 年分の履歴も行数に関係なく定数メモリで出力。",
      ],
      tech: ["Go 1.26", "Gin", "PostgreSQL 18", "pgx/v5", "sqlc", "golang-migrate"],
    ...
    },
    {
      ...
      demo: enProjects.projects[3].demo,
    },
    {
    ...
    {
      id: enProjects.projects[5].id,
      title: enProjects.projects[5].title,
      ...
      tech: enProjects.projects[5].tech,
      github: enProjects.projects[5].github,
    },
  ],
